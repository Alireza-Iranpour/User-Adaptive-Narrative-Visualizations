...
fill_results <- function(model_stats, model, results, iter_idx) {
  ...
  model_stats[iter_idx, paste(model, "_NumLow", sep="")] = sum(test_data[, dependent[dep]] == 0)
  model_stats[iter_idx, paste(model, "_NumHigh", sep="")] = sum(test_data[, dependent[dep]] == 1)
...
train_test_model <- function(model_name, tune_params, models_stats, feature_imp_fname, fold) {
  ...
  print(varImp(fit))
  sink() 
  res = predict(fit, newdata=test_data)
  res = confusionMatrix(res, test_data[,dependent[dep]])
...
user_dependent = c("Meara") # c("BarChartLit", "VerbalWM_longest", "Meara")
...
models = c("MajorityClass","rf")#"rf", "regLogistic", "xgbTree","svmLinear", "BstLm") #LogitBoost,
# Metrics to store
...
for (window in time_windows) {
  if (cumul_across) { # if across task task_1-task_15
    ...
    iter_arr = c(4)         # seq(1, 15, 1)                                                      # Across task: # task
  } else if (full_window) {
    features_dir = "C:\\Users\\Alire\\Desktop\\rcode\\train_data\\across_tasks_new_refined\\"
    ...
    for(dep in 1:length(dependent))
    {
      ...
      for (numRepeat in 1:num_repeats) {
        ...
        print(fold)
          ...
          ctrl1 <- trainControl(method = "cv", number = 10, allowParallel=T)   # for inner loop
        ...
        #  print(fit)
          #----------------------------------------------------
          # extract the hyper-parameter values which yielded the highest accuracy (best model at this iteration)
          
          best_model = which(fit$results$Accuracy == max(fit$results$Accuracy))
          best_model = best_model[1] # select the first one in case of multiple max 
          
          if(m==1){ # rf
            tune_param <- expand.grid(mtry = fit$results$mtry[best_model])
          }
          
          
          if(m==2){ # xgbTree
            tune_param <- expand.grid(nrounds = fit$results$nrounds[best_model], 
                                     max_depth = fit$results$max_depth[best_model],
                                     eta = fit$results$eta[best_model], 
                                     gamma = fit$results$gamma[best_model],
                                     colsample_bytree = fit$results$colsample_bytree[best_model],
                                     min_child_weight = fit$results$min_child_weight[best_model],
                                     subsample = fit$results$subsample[best_model]
                                    )
          }
          ...
          models_stats = train_test_model(model_name, tune_param, models_stats, feature_imp_fname, fold)     
          
        }
        
        #-------------------------------------------Generalized Accuracy---------------------------------------------------------

        Acc_overall = mean(models_stats[,paste(model_name,"_Accuracy", sep = "")])
        Acc_low = weighted.mean(models_stats[,paste(model_name,"_AccuracyLow", sep = "")], models_stats[,paste(model_name,"_NumLow", sep = "")])
        Acc_high = weighted.mean(models_stats[,paste(model_name,"_AccuracyHigh", sep = "")], models_stats[,paste(model_name,"_NumHigh", sep = "")])
        
        print(c("Overall Accuracy: ", Acc_overall))
        print(c("Low Accuracy: ", Acc_low))
        print(c("High Accuracy: ", Acc_high))
